...
Note that Vercel triggers a deployment every time its linked repo's main branch is pushed to. Since this repo is `my_username/my_heirloom_site` and not `my_username/my_content`, updating the markdown content will _not_ automatically rebuild your website. You probably want to set up a GitHub action on `my_username/my_content` that pings a Vercel Deploy Hook on every commit to automatically redeploy the website.
