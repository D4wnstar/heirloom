...
<AppBar class="bg-surface-100-900">
    <AppBar.Toolbar class="grid-cols-[auto_1fr_auto]">
        <AppBar.Lead>
            ...
            <Dialog>
                <Dialog.Trigger
                    class="btn-icon btn-icon-lg hover:preset-tonal [@media(min-width:1200px)]:hidden"
                >
                    <MenuIcon />
                </Dialog.Trigger>
                <Portal>
                    ...
                    <Dialog.Positioner class="fixed inset-0 z-50 flex justify-start">
                        <Dialog.Content
                            class="card bg-surface-100-900 h-screen w-sm space-y-4 rounded-none p-4 shadow-xl"
                        >
